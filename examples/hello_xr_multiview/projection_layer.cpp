...
void ProjectionLayer::initializeScene()
{
    ...
    // setup, this needs to be large enough to hold the data for all views (both eyes)
    {
        const BufferOptions bufferOptions = {
            .label = "Camera Buffer",
            .size = sizeof(glm::mat4) * 2 * viewCount(),
            .usage = BufferUsageFlagBits::UniformBufferBit,
            .memoryUsage = MemoryUsage::CpuToGpu // So we can map it to CPU address space
        };
...
void ProjectionLayer::updateViewUbo()
{
    ...
    std::memcpy(cameraBufferData, m_cameraData.data(), 2 * viewCount() * sizeof(glm::mat4));
    m_cameraBuffer.unmap();
}
